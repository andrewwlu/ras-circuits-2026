...
if (length(output) != 1) stop("Usage: Rscript fig4G_plot.R output.pdf")
dir.create(dirname(output), recursive = TRUE, showWarnings = FALSE)
script <- sub("^--file=", "", grep("^--file=", commandArgs(), value = TRUE))
d <- read.csv(file.path(dirname(normalizePath(script)), "fig4G_data.csv"), na.strings = "")
levels <- c("PBS", "Pyroptosis Circuit")
...
p <- ggplot() +
  ...
  geom_segment(data=brackets,aes(x=x,xend=x,y=y,yend=y-0.44),linewidth=.25) +
  geom_segment(data=brackets,aes(x=xend,xend=xend,y=y,yend=y-0.44),linewidth=.25) +
  ...
  scale_x_continuous(breaks=seq_along(levels),labels=c("PBS", "Pyroptosis\ncircuit"),expand=expansion(add=.6)) +
  scale_y_continuous(breaks=c(0,5,10,15,20),expand=expansion(mult=0)) +
  ...
  theme(legend.position="none",axis.text=element_text(color="black",size=7),
    ...
    plot.margin=margin(18,8,5,5))
p <- p + geom_hline(data=subset(d,layer=="reference"),aes(yintercept=y),linetype="dotted",linewidth=.3)
cairo_pdf(output,width=1.35,height=2.5); print(p); invisible(dev.off())
